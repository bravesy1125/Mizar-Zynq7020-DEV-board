...
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0)) */
#endif /* CUSTOM_BLOCK_DEAUTH_AT_EAP_FAILURE */
//	dhd_wait_pend8021x(dev); // terence 20180530: fix for GO connection issue for USB Wi-Fi
...
static s32 wl_inform_bss(struct bcm_cfg80211 *cfg)
{
	...
	if (cfg->autochannel)
		wl_ext_get_best_channel(ndev, &g_bss_cache_ctrl, &cfg->best_2g_ch, &cfg->best_5g_ch);
#else
	WL_SCAN(("scanned AP count (%d)\n", bss_list->count));
	preempt_disable();
	bi = next_bss(bss_list, bi);
...
static s32 wl_inform_single_bss(struct bcm_cfg80211 *cfg, struct wl_bss_info *bi, bool roam)
{
	...
	WL_SCAN(("BSSID %pM, channel %2d(%2d %sMHz), rssi %3d, capa 0x04%x, mgmt_type %d, "
		"frame_len %d, SSID \"%s\"\n",
		&bi->BSSID, notif_bss_info->channel, CHSPEC_CHANNEL(bi->chanspec),
		CHSPEC_IS20(bi->chanspec)?"20":CHSPEC_IS40(bi->chanspec)?"40":CHSPEC_IS80(bi->chanspec)?"80":"160",
